...
class UffizziCore::ComposeFileServiceTest < ActiveSupport::TestCase
  ...
  test '#build_template_attributes - check anonymous volumes' do
    ...
    assert_equal(UffizziCore::ComposeFile::Parsers::Services::VolumesService::ANONYMOUS_VOLUME_TYPE, first_volume[:type])
    assert_equal(content_data.dig('services', 'nginx', 'volumes').first, first_volume[:source])
    assert_nil(first_volume[:target])
  end
